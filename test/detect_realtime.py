...
def main(_argv):
    ...
    frame_num = 0
    start_compute_time = datetime.datetime.now()
    while True:
        ...
        if len(pending_task) < thread_num:
            ...
            if FLAGS.count:
                ...
                for key, value in counted_classes.items():
                    print("Jumlah {}: {} Buah".format(key, value))
    ...
    end_compute_time = datetime.datetime.now()
    print('elapsed time: %.2f' % (end_compute_time-start_compute_time).total_seconds())
    print("FPS: %.2f" % fps)
